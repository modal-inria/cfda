...
```{r genData, echo=TRUE}
K <- 4
QJK1 <- matrix(1/3, ncol = K, nrow = K) - diag(rep(1/3, K))
lambda_QJK1 = c(1, 1, 1, 1)

Tmax <- 10
n <- 100

d_JK1 <- generate_Markov_cfd (n = n, K = 4, Q = QJK1, lambda = lambda_QJK1, Tmax = Tmax)
...
Some methods requires that the end of each individuals is $T_{\max}$, this can be done with the following function:

```{r cutT, echo=TRUE}
d_JK1T <- msm2msmTmax(d_JK1, Tmax = Tmax)
```


A trajectory of one indivuals can be plotted: 
```{r plot, echo=TRUE}
plot(d_JK1[d_JK1$id == 1, c("time", "state")], type = "S", yaxt = "n", bty = "n", lwd = 3)
axis(2, 1:4, c("A", "T", "C", "G"), las = 2)
...
```{r, echo=TRUE}  
timeSpent <- compute_Time_Spent(d_JK1T, K)
timeSpent[1:10,]
```
...
```{r, echo=TRUE}  
nJump <- compute_number_jumps(d_JK1)
head(nJump)
``` 
...
```{r, echo=TRUE}
fmca <- compute_optimal_encoding(d_JK1T, b)
```

The output is a list containing the different elements computed during the process: `eigenvalues`, `pc`, `alpha`, `F`, `G` and `V`.

```{r, echo=TRUE}
 barplot(fmca$eigenvalues, main = "Eigenvalues")
```
